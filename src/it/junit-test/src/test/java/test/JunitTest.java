...
public class JunitTest {
    ...
    @Test
    public void testZoneRulesGetOffsetLongSydney2000Jan1() throws Exception {
        final Date date = new Date(Date.UTC(2000 - 1900, Calendar.JULY, 1, 0, 59, 0));
        Assert.assertEquals("getOffset(long) " + date,
                36000000,
                this.standardZoneRules().getOffset(date.getTime()));
    }

    @Test
    public void testZoneRulesGetOffset5IntSydney2000Jan1() throws Exception {
        final int year = 2000;
        final int month = Calendar.JULY;
        final int day = 1;
        final int hours = 1;
        final int minutes = 59;
        final Date date = new Date(year - 1900, month, day, hours, minutes, 0);

        Assert.assertEquals("getOffset(" + year + "/" + month + "/" + day + " " + hours + ":" + minutes + ") " + date,
                36000000,
                this.standardZoneRules().getOffset(GregorianCalendar.AD,
                        year,
                        month,
                        day,
                        Calendar.MONDAY,
